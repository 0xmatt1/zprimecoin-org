...
Jack Grigg (22):
      ...
      Instruct users to run zprime-fetch-params if network params aren't available
      ...
      Suggest ./zputil/fetch-params.sh as well
      Update debug categories
      Rename build-aux/m4/bitcoin_find_bdb48.m4 to remove version
      Throw an error if zprime.conf is missing
      Show a friendly message explaining why zprimed needs a zprime.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty zprime.conf during performance measurements
      Create empty zprime.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from zprime.conf to avoid problems with command line tools
      Closes #1097 so zprime-cli now displays license info like zprimed.
