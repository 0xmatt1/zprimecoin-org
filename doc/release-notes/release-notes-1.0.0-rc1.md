...
Jay Graber (17):
      ...
      Refer to zPrime wiki in INSTALL
      Rm bitcoin logo
      Rm build-unix.md, to keep single copy of build instructions for zPrime on github wiki
      Rm Bitcoin-specific documentation
      Add note that document is not updated for zPrime to translation policy
      Rm doc for disabled REST interface
      Change alpha to beta testnet, add zprime hidden service
      Improve documentation on connecting to zprime hidden server
      Improve documentation on connecting to zprime hidden server
      Update tor.md
...
Kevin Gallagher (24):
      ...
      Adds zprime/zPrime.h to LIBZPRIME sources
      Adds zprime/Proof.hpp to LIBZPRIME_H
      Add alertkeys.h to libbitcoin_server_a_SOURCES
      Adds files in src/zprime/circuit to libzprime_a_SOURCES
      Adds zcbenchmarks.h to libbitcoin_wallet_a_SOURCES
      Adds json_test_vectors.h to zprime_gtest_SOURCES
      Adds additional licenses to Debian copyright file
      Updates zPrime Core developers -> zPrime developers
...
Simon (11):
      ...
      Reorder gtests in zprime-gtest.
