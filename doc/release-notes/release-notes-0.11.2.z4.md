...
Alfie John (1):
      Rename libzerocash to libzprime
...
Jack Grigg (21):
      ...
      Add zPrime revision to version strings
      Bitcoin -> zPrime in version and help text
      Add zPrime Developers to CLI copyright notice
...
Nathan Wilcox (1):
      Fix a test name bug so that ``make cov-zprime`` correctly runs the ``zprime-gtest`` binary. Fixes #946.
...
Taylor Hornby (16):
      ...
      Turn on -Werror for the zPrime build.
