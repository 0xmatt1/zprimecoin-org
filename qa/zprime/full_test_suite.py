#!/usr/bin/env python2
#
# Execute all of the automated tests related to zPrime.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/zprime/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/zprime/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if zprimed is an ELF binary, they all are
    with open(repofile('src/zprimed'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/zprimed')
    ret &= test_rpath_runpath('src/zprime-cli')
    ret &= test_rpath_runpath('src/zprime-gtest')
    ret &= test_rpath_runpath('src/zprime-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/zprimed')
    ret &= test_fortify_source('src/zprime-cli')
    ret &= test_fortify_source('src/zprime-gtest')
    ret &= test_fortify_source('src/zprime-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/zprime-gtest')],
