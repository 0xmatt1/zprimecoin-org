...
#ifndef ZPRIME_PAYMENTDISCLOSURE_H
#define ZPRIME_PAYMENTDISCLOSURE_H
...
struct PaymentDisclosureInfo {
    ...
    uint256 esk;              // zprime/NoteEncryption.cpp
    uint256 joinSplitPrivKey; // primitives/transaction.h
    // ed25519 - not tied to implementation e.g. libsodium, see ed25519 rfc

    libzprime::SproutPaymentAddress zaddr;

    PaymentDisclosureInfo() : version(PAYMENT_DISCLOSURE_VERSION_EXPERIMENTAL) {
    }

    PaymentDisclosureInfo(uint8_t v, uint256 esk, uint256 key, libzprime::SproutPaymentAddress zaddr) : version(v), esk(esk), joinSplitPrivKey(key), zaddr(zaddr) { }
...
struct PaymentDisclosurePayload {
    ...
    libzprime::SproutPaymentAddress zaddr; // zprime/Address.hpp
...
#endif // ZPRIME_PAYMENTDISCLOSURE_H
