...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"zprimeprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"zprimeprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid zPrime address or script");
    }
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid zPrime spending key
        if (fImportZKeys) {
            ...
            if (IsValidSpendingKey(spendingkey)) {
                ...
                // Not a valid spending key, so carry on and see if it's a zPrime style t-address.
            }
        }
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by zprimed -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the zprimed -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by zPrime %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    if (fDumpZKeys) {
        std::set<libzprime::SproutPaymentAddress> sproutAddresses;
        ...
        std::set<libzprime::SaplingPaymentAddress> saplingAddresses;
...
UniValue z_importviewingkey(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libzprime::SproutViewingKey>(&viewingkey) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Currently, only Sprout viewing keys are supported");
    }
    auto vkey = boost::get<libzprime::SproutViewingKey>(viewingkey);
...
UniValue z_exportviewingkey(const UniValue& params, bool fHelp)
{
    ...
    // TODO: Add Sapling support. For now, return an error to the user.
    if (boost::get<libzprime::SproutPaymentAddress>(&address) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Currently, only Sprout zaddrs are supported");
    }
    auto addr = boost::get<libzprime::SproutPaymentAddress>(address);

    libzprime::SproutViewingKey vk;
