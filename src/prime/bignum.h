...
// Copyright (c) 2019 The zPrime developers
...
public:
    ...
    CBigNum nthRoot(int n) const
    {
        ...
        for (int it = 0; it < 20; it++)
        {
            ...
            if (BN_is_zero(bnDelta.get()))
                return bnCur;
            if (BN_is_negative(bnDelta.get()))
            {
                if (nTerminate == 1)
                    return bnCur - 1;
                BN_set_negative(bnDelta.get(), 0);
                if (BN_cmp(bnDelta.get(), bnRoot.get()) <= 0)
                {
                    bnCur -= 1;
                    nTerminate = -1;
                    continue;
                }
                BN_set_negative(bnDelta.get(), 1);
            }
            else
            {
                if (nTerminate == -1)
