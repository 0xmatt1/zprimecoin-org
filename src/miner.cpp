...
static bool ProcessBlockFound(CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("zPrimeMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("zPrimeMiner: ProcessNewBlock, block not accepted");
...
{
    LogPrintf("zPrimeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zprime-miner");
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in zPrimeMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in zPrimeMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running zPrimeMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("zPrimeMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("zPrimeMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
