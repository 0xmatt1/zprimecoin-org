...
public:
    ...
    libzprime::GrothProof zkproof;  //!< A zero-knowledge proof using the spend circuit.
...
public:
    ...
    libzprime::SaplingEncCiphertext encCiphertext; //!< A ciphertext component for the encrypted output note.
    libzprime::SaplingOutCiphertext outCiphertext; //!< A ciphertext component for the encrypted output note.
    libzprime::GrothProof zkproof;   //!< A zero-knowledge proof using the output circuit.
...
public:
    ...
    void operator()(const libzprime::PHGRProof& proof) const
    {
        if (useGroth) {
    ...
    void operator()(const libzprime::GrothProof& proof) const
    {
        if (!useGroth) {
...
{
    if (useGroth) {
        libzprime::GrothProof grothProof;
        ::Unserialize(s, grothProof);
        proof = grothProof;
    } else {
        libzprime::PHGRProof pghrProof;
...
public:
    ...
    JSDescription(
            ...
            const uint256& rt,
            const std::array<libzprime::JSInput, ZC_NUM_JS_INPUTS>& inputs,
            const std::array<libzprime::JSOutput, ZC_NUM_JS_OUTPUTS>& outputs,
            CAmount vpub_old,
            CAmount vpub_new,
    ...
    bool Verify(
        ZCJoinSplit& params,
        libzprime::ProofVerifier& verifier,
        const uint256& joinSplitPubKey
    ) const;
