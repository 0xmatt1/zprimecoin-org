...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total zPrime balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"zprimeaddress\"\n"
            "\nReturn information about the given zPrime address.\n"
            "\nArguments:\n"
            "1. \"zprimeaddress\"     (string, required) The zPrime address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"zprimeaddress\",   (string) The zPrime address validated\n"
...
{
public:
    UniValue operator()(const libzprime::InvalidEncoding &zaddr) const { return UniValue(UniValue::VOBJ); }

    UniValue operator()(const libzprime::SproutPaymentAddress &zaddr) const {
        UniValue obj(UniValue::VOBJ);
    ...
    UniValue operator()(const libzprime::SaplingPaymentAddress &zaddr) const {
        UniValue obj(UniValue::VOBJ);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are zPrime addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) zPrime address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"zprimeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"zprimeaddress\"    (string, required) The zPrime address to use for the signature.\n"
