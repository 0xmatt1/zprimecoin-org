#!/bin/bash
## Usage:
##  ./zputil/build-debian-package.sh
...
PACKAGE_VERSION=$($SRC_PATH/src/zprimed --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/zprimed $DEB_BIN
cp $SRC_PATH/src/zprime-cli $DEB_BIN
cp $SRC_PATH/zputil/fetch-params.sh $DEB_BIN/zprime-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/zprimed.1 $DEB_MAN
cp $SRC_DOC/man/zprime-cli.1 $DEB_MAN
cp $SRC_DOC/man/zprime-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/zprimed.bash-completion $DEB_CMP/zprimed
cp $SRC_PATH/contrib/zprime-cli.bash-completion $DEB_CMP/zprime-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/zprimed.1
gzip --best -n $DEB_MAN/zprime-cli.1
gzip --best -n $DEB_MAN/zprime-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/zprimed $DEB_BIN/zprime-cli
