...
def main(args=sys.argv[1:]):
    """
    Perform the final zPrime release process up to the git tag.
    """
...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libzprimeconsensus.pc',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "zprime-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "zprime-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './zprime-make-release.log'
    ...
    logging.info('zprime make-release.py debug log: %r', logname)
